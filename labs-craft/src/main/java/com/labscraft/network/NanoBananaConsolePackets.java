...
public class NanoBananaConsolePackets {

    public record GeneratePayload() implements CustomPayload {
        public static final Id<GeneratePayload> ID = new Id<>(Identifier.of(LabsCraft.MOD_ID, "nano_banana_generate"));
    ...
    public static void registerServer() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(GeneratePayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                if (context.player().currentScreenHandler instanceof NanoBananaConsoleScreenHandler handler) {
