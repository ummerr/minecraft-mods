...
public class FlowConsolePackets {

    public record GeneratePayload() implements CustomPayload {
        public static final Id<GeneratePayload> ID = new Id<>(Identifier.of(LabsCraft.MOD_ID, "generate"));
    ...
    public static void registerServer() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(GeneratePayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                if (context.player().currentScreenHandler instanceof FlowConsoleScreenHandler handler) {
